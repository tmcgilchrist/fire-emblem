...
*)
let rec cap lst s =
  match lst with [] -> 40 | h :: t -> if fst h = s then snd h else cap t s
...
*)
let equipped c = if c.eqp = -1 then None else c.inv.(c.eqp)
...
*)
let comp a b = lv_to_int a >= lv_to_int b
...
let rec prof wlvlst i =
  ...
  | [] -> false
  | (x, y, z) :: t ->
      if i.wtype = x then if comp y i.level then true else false else prof t i
...
*)
let equippable c i =
  match i.wtype with Staff | Potion | Key -> false | x -> prof c.wlevels i
...
let rec equip_id c n =
  ...
  | Some x ->
      if equippable c x then n else if n = 4 then -1 else equip_id c (n + 1)
...
let stat_up (c : character) s i =
  ...
  | Health ->
      ...
      if from_cap < i then
        c.health <- (fst c.health + from_cap, snd c.health + from_cap)
      else c.health <- (fst c.health + i, snd c.health + i)
  | Str ->
      let str = c.str + i in
...
let update_health (c : character) i =
  ...
  if fst c.health - i > snd c.health then
    c.health <- (snd c.health, snd c.health)
...
let add_item c i = add_helper c.inv i 0
...
let rec update_character c =
  ...
  if fst c.health < 0 then (
    c.health <- (0, snd c.health);
    update_character c);
  if fst c.health > snd c.health then (
    c.health <- (snd c.health, snd c.health);
    update_character c);
  ...
  let calc_hit =
    if e = -1 then 0
      ...
      match c.inv.(e) with None -> 0 | Some x -> x.acc + (2 * c.skl) + c.lck
  in
  let calc_atk =
    if e = -1 then 0
    else
  ...
  let calc_crit =
    if e = -1 then 0
    else match c.inv.(e) with None -> 0 | Some x -> x.crit + (c.lck / 2)
  in
...
*)
let remove_item a i = a.inv.(i) <- None
