...
let init_state =
  ...
  in
  x
  |> set_init_ch_movement x.player
  |> set_init_ch_movement x.enemies
  |> set_act_tile
...
let main () =
  ...
  append_text p2
    "Developed by: Frank Rodriguez, Albert Tsao, Darren Tsai, and Ray Gu";
   ...
   * lift events. *)
  let _ =
    Html.addEventListener document Html.Event.keydown
      (Html.handler Command.keydown)
      Js._true
  in
